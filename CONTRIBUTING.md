...
- **No reflection** in the display engines at all. The
  `ColorDisplayManagerEngine` that used to be the one exception was removed;
  the rootless driver now uses public `Settings.Secure` calls. Elsewhere,
...
- Run `./gradlew test` before pushing. For UI or theme changes, also run
  ...
  The local release gate (`py -3 tools/local_release_gate.py`) runs the
  release build, lint, module tests, screenshot lanes, manifest permission
  checks, classpath checks, SBOM/advisory output, and signature verification;
  run it before release tags.
...
- **Never assume reflection succeeded.** AOSP private APIs drift; the
  the removed `ColorDisplayManagerEngine` reflection layer had to keep its
