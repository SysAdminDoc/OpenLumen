...
4. Risk: most Android TV devices have heavily customized firmware that
   ignores the rootless `SecureSettings` driver. The Driver tab's per-engine status
...
Static pass on 2026-06-15 improved recoverability and visual consistency:
profile deletion now has undo recovery, diagnostics/location chips use the
project shape system, widget labels use larger centered ellipsized text, and
stale light-theme screenshot baselines were refreshed. The device-scanner and
font-scale evidence below remains pending.
...
(`SecureSettings`, `KCAL`, `Overlay`) keep the older
...
## LumenService subsystem split: C167 / C195 (shipped in v0.6.2)

**Status**: shipped 2026-06-27.

The foreground service now stays as the Android lifecycle and intent
orchestrator while five collaborators own the stateful mechanics:

1. **`EngineController`** owns active engine selection, serialized
   `apply()` / `clear()` calls, transition ramps, hard reset, and
   Locked Boot matrix application.
2. **`ScheduleAlarmOrchestrator`** owns the `AlarmManager`
   `PendingIntent`, next-transition scheduling, alarm cancellation,
   and next-alarm-clock lookup.
3. **`LightSensorSubscription`** owns the light-sensor collection job,
   latest-lux cache, screen-off invalidation, and re-evaluation callback.
4. **`WidgetBridge`** owns widget-visible preference diffing before it
   broadcasts Glance refreshes.
5. **`DirectBootMirror`** owns device-protected mirror writes, disabled
   Locked Boot state updates, snapshot reads, and `LumenMatrix` /
   `MatrixDto` conversion.

`LumenService.kt` dropped from roughly 1,050 lines to roughly 640 lines.
The new `WidgetBridgeTest` covers widget-refresh diffing and
`DirectBootMirrorTest` covers full matrix round-tripping for Locked Boot
restore.
