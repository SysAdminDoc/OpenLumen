...
| **AOSP Night Light surface** | None. `ColorDisplayService` is unchanged in Android 17 (S138 negative confirmation), so the secure rows the `SecureSettings` driver writes need no Android-17-specific case. The `ColorDisplayManager` reflection this row used to name was removed. | N/A | S138 |
...
- **C95** — AGP 9 migration spike. Shipped 2026-05-17 on AGP 9.2.1 /
  Gradle 9.4.1 / Kotlin 2.3.21. Kotlin is 2.4.10 now (C264).
- **C96 / C124** — Hilt Compose artifact rename and Hilt 2.56+ bump.
  Shipped 2026-05-17 on Hilt 2.59.2. Hilt is 2.60.1 now (C264).
