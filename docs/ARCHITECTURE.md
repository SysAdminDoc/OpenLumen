...
Direct Boot is the only intentional mirror. `DirectBootMirror` writes a small
...
Inside `LumenService` and its service-package collaborators:

- `EngineController.applyMutex` serializes every `engine.apply()` and
  `engine.clear()` call. Two concurrent `su` subprocesses would step on each
  other, especially for KCAL where each call writes multiple sysfs files.
- `EngineController.rampMutex` serializes smooth-transition cancel/join/launch
  state so prefs emissions, light-sensor emissions, engine swaps, and manual
  off actions cannot leave an old ramp coroutine applying over the latest
  target.
- `LumenService.latestPrefs` and `LightSensorSubscription.latestLux` hold the
  most recent snapshots. The alarm receiver and sensor flow read these without
  coupling to the prefs-flow collector coroutine.
...
- **`SecureSettingsEngine`** — public `Settings.Secure.putInt` against the
  `night_display_*` and `reduce_bright_colors_*` rows `ColorDisplayService`
  observes. Replaced a `ColorDisplayManager` reflection engine that could not
  work on a user install, because those setters need
  `CONTROL_DISPLAY_COLOR_TRANSFORMS` and `pm grant` cannot grant it. Cached after first
  successful load. Gated on `WRITE_SECURE_SETTINGS` being granted.
- **`SurfaceFlingerEngine`** — `service call SurfaceFlinger <code>` via `su`.
  The colour matrix is transaction 1015, unchanged from `android-10.0.0_r1`
  through `android-16.0.0_r1`. The probe issues the disable transaction and
  reads the reply parcel, because `service call` exits 0 whether or not the
  service accepted it.
...
5. Wire the new DTO value through `EngineController.ensureEngineFor()` and
...
`LumenService` uses `isActive` for the current apply decision.
`ScheduleAlarmOrchestrator` uses `nextTransition` to set the next
`AlarmManager` alarm.
...
- **Reflection drift.** No longer applicable: the reflection engine this
  described was removed. Kept here so a reader of an older build knows why.
  The former text follows.

  `ColorDisplayManagerEngine` reflected against a hidden
  AOSP API. Future Android versions can rename methods; the `runCatching`
  ladder is defensive on purpose.
- **A rejected `service call SurfaceFlinger` transaction.** The colour matrix
  is 1015 and has not moved. Earlier releases also probed 1023, 1030 and 1036;
  none was ever verified against AOSP, and 1023 is the colour mode. Add a code
  to `SurfaceFlingerEngine.CANDIDATE_CODES` only with a citation to the
  `onTransact` case that implements it.
- **Overlay window token.** `TYPE_APPLICATION_OVERLAY` needs a Service or
  Activity context; `Application` context throws. `EngineController` calls
  `overlay.installView(serviceContext, …)` before the first apply.
