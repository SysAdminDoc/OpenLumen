...
Pulse-width modulation (PWM) is a display implementation detail. Its
behavior varies by panel, firmware, refresh rate, and hardware-brightness
range, so OpenLumen cannot promise that a particular setting will feel the
same on another device.

For users exploring an overlay-at-higher-hardware-brightness workflow:

1. Select the **Overlay** driver and grant its display-over-other-apps
   permission.
2. Try the **PWM Comfort** preset, or choose a custom warm tint and use the
   Home tab's Dim control for a smaller adjustment. The overlay driver's
   Android alpha cap still applies.
3. Set the device's hardware brightness to the level that looks right to you,
   then adjust OpenLumen's overlay dim separately. Keep an eye on battery use
   and heat because a brighter panel setting can consume more power.
4. If comparing this with OLED Saver / Screen Dimmer (S103) or Iris (S23),
   test one overlay at a time. Pause OpenLumen before enabling the other tool
   so stacked color and dim layers do not make the result difficult to reason
   about. The two tools have different permissions and rendering behavior.

This is user experimentation for display comfort, not medical advice and not
a claim that any setting treats PWM sensitivity, migraine, eye strain, or any
other condition. OpenLumen intentionally has no AccessibilityService backend
and cannot guarantee OLED behavior on a particular device.
...
- **C62** (Research-based preset labels): potential rename of "Deep Sleep"
  ...
  consideration.
- **C100** (Medical/pain-mode disclaimer templates): the PWM workflow above
  is explicitly comfort-oriented user guidance, not medical advice.
...
- **S47** — Sleep Advances review of blue-light interventions.
  ...
  quality.
- **S23** — Iris, https://iristech.co/iris/ — reference for a separate
  PWM-sensitive display workflow; OpenLumen does not reproduce its
  AccessibilityService or partial-screen approach.
- **S103** — OLED Saver / Screen Dimmer,
  https://play.google.com/store/apps/details?id=dev.rewhex.screendimmer —
  reference for an overlay-at-higher-hardware-brightness workflow, not a
  health-efficacy source.
