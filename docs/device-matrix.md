...
## SecureSettings specifics

Recorded against the `SecureSettings` driver. Rows captured before 0.8.0 name
a `ColorDisplayManager` driver that was removed; it could not work on a user
install and the secure-settings writer replaced it.
...
- Requires `su`. Confirm `adb shell which su` returns a path before
  marking ✅.
- The probe uses the single sourced transaction code 1015. If
