...
## Implementation update (C120)

C120 used the existing F-Droid reproducible-build sources plus a primary
AGP DSL reference:

- F-Droid documents `version-control-info.textproto` as VCS metadata
  generated by AGP 8.3+ and recommends building release APKs from a clean
  tagged commit; only exceptional cases should disable the file (S112/S156).
- AGP 8.7 exposes `VcsInfo.include` as the release-build toggle (S268).
